...
\_changes. For example, using feed=longpoll or continuous doesn't make
much sense within R itself.
...
- feed:
  ...
  Select the type of feed. One of normal, longpoll, or continuous. See
  description. (character)

- heartbeat:

  Period in milliseconds after which an empty line is sent in the
  results. Only applicable for longpoll or continuous feeds. Overrides
  any timeout to keep the feed alive indefinitely. (numeric
  (milliseconds))
...
- results - Changes made to a database, length 0 if no changes. Each of
  these has:
